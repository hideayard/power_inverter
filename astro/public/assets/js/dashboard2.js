...
function convertDevicesToNodes(devices) {
  return devices.map((device, index) => {
    ...
    // Get device name
    const deviceName = device.device_alias || 
                      device.device_name || 
                      device.telemetry?.device_name_from_telemetry || 
                      `Device ${index + 1}`;

    // Get location text
    const locationText = device.location?.formatted || 
...
async function initializeDataFromServer() {
  try {
    ...
    if (NODES.length === 0) {
      console.warn("No devices found from server, using sample data");
      useSampleData();
      return;
    }
    ...
    NODES.forEach((node) => {
      ...
      const voltage = hasTelemetry ? parseFloat(device.telemetry.ac_voltage || "230.0") : 230.0;
      const current = hasTelemetry ? parseFloat(device.telemetry.ac_current || "1.5") : 1.5;
      const pf = hasTelemetry ? parseFloat(device.telemetry.power_factor || "0.95") : 0.95;
      ...
      const frequency = hasTelemetry ? parseFloat(device.telemetry.frequency || "50.0") : 50.0;
      
      // Calculate battery percentage from metrics or DC voltage
      let batterySoC = 0.7; // Default
...
function updateNodeData() {
  // Update each node's data with realistic fluctuations
  NODES.forEach((node) => {
    const nodeData = nodesData[node.id];
    const device = node.raw_device;
    const hasTelemetry = device && device.telemetry;
    
    // Base values from server data
    let baseVoltage = 230.0;
    let baseCurrent = 1.5;
    let basePF = 0.95;
    let basePower = 0;
    let baseFrequency = 50.0;
    let baseEnergy = nodeData.energy;
    
    if (hasTelemetry) {
      baseVoltage = parseFloat(device.telemetry.ac_voltage || "230.0");
      baseCurrent = parseFloat(device.telemetry.ac_current || "1.5");
      basePF = parseFloat(device.telemetry.power_factor || "0.95");
      basePower = parseFloat(device.telemetry.ac_power || "0");
      baseFrequency = parseFloat(device.telemetry.frequency || "50.0");
      baseEnergy = parseFloat(device.telemetry.energy || "0");
    }
    
    // Add small random fluctuations to simulate real-time updates
    const fluctuation = Math.random() * 0.1 - 0.05; // ±5% fluctuation
    
    nodeData.voltage = Math.max(200, Math.min(250, baseVoltage * (1 + fluctuation)));
    nodeData.current = Math.max(0, baseCurrent * (1 + fluctuation));
    nodeData.pf = Math.min(1, Math.max(0.7, basePF * (1 + fluctuation * 0.5)));
    
    // Calculate power if not provided, otherwise use provided power with fluctuation
    if (basePower > 0) {
      nodeData.power = parseFloat((basePower * (1 + fluctuation)).toFixed(1));
    } else {
      nodeData.power = parseFloat((nodeData.voltage * nodeData.current * nodeData.pf).toFixed(1));
    }
    
    // Update energy based on power (2 seconds worth)
    const energyIncrement = (nodeData.power * 0.000556) / 1000; // Convert to kWh
    nodeData.energy = parseFloat((baseEnergy + energyIncrement).toFixed(3));
    
    // Update battery
    if (nodeData.batterySoC > 0) {
      // Small discharge based on power consumption
      const dischargeRate = nodeData.power > 100 ? 0.0002 : 0.00005;
      nodeData.batterySoC = Math.max(0, nodeData.batterySoC - dischargeRate);
    }
    
    // Update frequency with small fluctuation
    nodeData.frequency = Math.max(49.5, Math.min(50.5, baseFrequency + (Math.random() * 0.2 - 0.1)));
    
    // Update carbon reduction
    nodeData.carbonReduction = parseFloat((nodeData.energy * CARBON_REDUCTION_FACTOR).toFixed(2));
    
    // Update battery time
    nodeData.batteryTime = calculateBatteryTime(nodeData.batterySoC);
    
    nodeData.lastUpdated = Date.now();
    ...
    historicalData[node.id].push({
      timestamp: Date.now(),
      timeString,
      voltage: parseFloat(nodeData.voltage.toFixed(2)),
      current: parseFloat(nodeData.current.toFixed(3)),
      power: parseFloat(nodeData.power.toFixed(1)),
      pf: parseFloat(nodeData.pf.toFixed(3)),
    });
...
function updateChartForAggregate() {
  ...
  const aggregateData = timeLabels.map((_, index) => {
    if (viewMode === "avg") {
      ...
      const voltages = nodeIds.map((id) => historicalData[id]?.[index]?.voltage || 0);
      const currents = nodeIds.map((id) => historicalData[id]?.[index]?.current || 0);
      const powers = nodeIds.map((id) => historicalData[id]?.[index]?.power || 0);
      const pfs = nodeIds.map((id) => historicalData[id]?.[index]?.pf || 0);

      return {
        voltage: voltages.reduce((a, b) => a + b, 0) / Math.max(voltages.length, 1),
        current: currents.reduce((a, b) => a + b, 0) / Math.max(currents.length, 1),
...
function getSelectedMetrics() {
  ...
  const checkboxes = paramSelector.querySelectorAll(
    'input[type="checkbox"]'
  );
  const selected = [];
